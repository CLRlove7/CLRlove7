...
> 📌脱敏专利摘要 
> 本发明涉及软件工程技术领域，涉及软件测试、单元测试、深度学习、人工智能等技术领域
...
单元测试是软件开发的基础环节，但传统方法存在以下挑战：

1. **耗时耗力**：测试人员需要先理解函数功能、分支、边界条件与异常状况
2. **覆盖率不高**：人工编写测试用例容易遗漏重要边界条件和特殊场景
3. **缺乏系统性**：不同测试人员对测试用例的理解和设计思路不同，质量参差不齐
4. **环境依赖**：自动化测试受被测单元和测试环境影响大，变化后需重新调整
5. **大模型幻觉**：纯生成式方法在代码任务中存在生成幻觉问题
